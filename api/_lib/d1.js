...
const D1_URL = (process.env.D1_REST_URL || process.env.D1_REST_ENDPOINT || process.env.D1_URL || '').replace(/\/$/, '');
const D1_TOKEN = process.env.D1_REST_TOKEN || process.env.D1_SECRET || process.env.token || process.env.TOKEN || '';
...
export async function ensureSchema() {
  ...
  schemaPromise = (async () => {
    await d1Query(`CREATE TABLE IF NOT EXISTS app_users (
      id TEXT PRIMARY KEY,
      name TEXT DEFAULT '',
    ...
    await d1Query(`CREATE TABLE IF NOT EXISTS bookmarks (
    ...
    await d1Query(`CREATE TABLE IF NOT EXISTS notebooks (
    ...
    await d1Query(`CREATE TABLE IF NOT EXISTS codes (
    ...
    await d1Query(`CREATE TABLE IF NOT EXISTS questions (
    ...
    await d1Query(`CREATE TABLE IF NOT EXISTS categories (
    ...
    await d1Query(`CREATE TABLE IF NOT EXISTS routines (
    ...
    await d1Query(`CREATE TABLE IF NOT EXISTS ai_settings (
    ...
    await d1Query(`CREATE TABLE IF NOT EXISTS chat_history (
      id TEXT PRIMARY KEY, userId TEXT NOT NULL UNIQUE, messages TEXT DEFAULT '[]',
      createdAt TEXT NOT NULL, updatedAt TEXT NOT NULL
    );`);

    await d1Query(`CREATE INDEX IF NOT EXISTS idx_bookmarks_user_created ON bookmarks(userId, createdAt);`);
    await d1Query(`CREATE INDEX IF NOT EXISTS idx_notebooks_user_updated ON notebooks(userId, updatedAt);`);
    await d1Query(`CREATE INDEX IF NOT EXISTS idx_codes_user_created ON codes(userId, createdAt);`);
    await d1Query(`CREATE INDEX IF NOT EXISTS idx_questions_user_created ON questions(userId, createdAt);`);
    await d1Query(`CREATE INDEX IF NOT EXISTS idx_routines_user_time ON routines(userId, dayOfWeek, date, startTime);`);
  })();
