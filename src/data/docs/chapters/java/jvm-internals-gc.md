...
জাভার সবচেয়ে বড় শক্তি হলো এর ভার্চুয়াল মেশিন (JVM)। জাভা সোর্স কোড সরাসরি মেশিনের নেটিভ কোডে কম্পাইল না হয়ে বাইটকোডে (`.class`) রূপান্তরিত হয়, যা যেকোনো প্ল্যাটফর্মে JVM-এর সাহায্যে রান করতে পারে ("Write Once, Run Anywhere")।
...
## ২. রানটাইম মেমোরি বিভাগ: Stack বনাম Heap
...
JVM-এর মেমোরি ম্যানেজমেন্ট **Weak Generational Hypothesis** এর ওপর প্রতিষ্ঠিত:
> *"অধিকাংশ অবজেক্ট তৈরির কিছুক্ষণের মধ্যেই অপ্রয়োজনীয় হয়ে মারা যায়।"*
...
### মেমোরি লাইফসাইকেল:
1. সব নতুন অবজেক্ট **Eden Space**-এ তৈরি হয়।
2. Eden ভরে গেলে একটি দ্রুতগতির **Minor GC** রান করে। বেঁচে থাকা অবজেক্টগুলো Survivor Space (S0)-এ পাঠানো হয়।
3. অবজেক্টগুলো S0 এবং S1 এর মধ্যে অল্টারনেট করে এবং প্রতিবার তাদের বয়স (Tenuring threshold) বাড়ে।
4. পর্যাপ্ত বয়স (ডিফল্ট ১৫ সাইকেল) পার হলে অবজেক্টগুলো **Old Generation**-এ স্থানান্তরিত (Promote) হয়।
...
| :--- | :--- | :--- | :--- | :--- |
| **G1 GC** (Garbage-First) | Region-based, Generational | Java 9+ | ২০০ মিলিসেকেন্ডের নিচে | মাল্টি-গিগাবাইট হিপ, ব্যালেন্সড থ্রুপুট |
...
### ফ্ল্যাগের ব্যাখ্যা:
- `-Xms4g -Xmx4g`: ইনিশিয়াল এবং ম্যাক্সিমাম হিপ সাইজ সমান রাখা হয়েছে, যাতে রানটাইমে হিপ রিসাইজিংয়ের কারণে JVM ওভারহেড না হয়।
- `-XX:+UseG1GC`: G1 কালেক্টর সক্রিয় করা।
- `-XX:MaxGCPauseMillis=200`: GC পজ টাইম ২০০ মিলিসেকেন্ডের মধ্যে রাখার সফট গোল নির্ধারণ।
- `-XX:+HeapDumpOnOutOfMemoryError`: মেমোরি লিকের কারণে ক্র্যাশ করলে সাথে সাথে মেমোরি অ্যানালাইসিস ডাম্প ফাইল তৈরি করবে।
- `-Xlog:gc*`: আধুনিক ইউনিফায়েড GC লগিং যা প্রোডাকশন পারফরম্যান্স পর্যবেক্ষণে ব্যবহৃত হয়।

---

## ৬. মেমোরি লিক শনাক্তকরণ ও প্রতিরোধ

জাভাতে মেমোরি লিক বলতে বোঝায় এমন কোনো অবজেক্ট যা প্রোগ্রামের লজিকের আর কোনো কাজে লাগবে না, কিন্তু কোনো সক্রিয় রুট রেফারেন্সের সাথে যুক্ত থাকায় GC তাকে ডিলিট করতে পারছে না।
...
4. **ThreadLocal Variable Leaks**: থ্রেড পুলে টাস্ক শেষে `ThreadLocal.remove()` কল না করা।
