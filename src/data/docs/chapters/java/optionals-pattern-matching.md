...
`Optional` হলো একটি সিঙ্গেল-ভ্যালু কন্টেইনার, যা একটি নন-নাল object ধারণ করতে পারে অথবা সম্পূর্ণ খালি (`empty`) থাকতে পারে। এর মূল উদ্দেশ্য হলো method-এর রিটার্ন টাইপ হিসেবে ব্যবহৃত হয়ে কলারকে স্পষ্টভাবে সতর্ক করা যে মানটি অনুপস্থিত থাকতে পারে।
...
> 2. ফিল্ড টাইপ বা method parameter হিসেবে `Optional` ব্যবহার করবেন না। এটি সিরিয়ালাইজেবল নয় এবং memory ফুটপ্রিন্ট বাড়ায়।

---

## ২. Pattern Matching for `instanceof` (Java 16+)

ক্লাসিক জাভাতে টাইপ চেক করার পর ম্যানুয়ালি Type Casting করতে হতো। আধুনিক জাভায় টাইপ প্যাটার্ন ম্যাচিংয়ের মাধ্যমে কাস্টিং স্বয়ংক্রিয় হয়ে গেছে:
...
Java 21-এ `switch` স্টেটমেন্ট এবং এক্সপ্রেশন শুধু প্রিমিটিভ ও string নয়, যেকোনো object টাইপের উপর প্যাটার্ন ম্যাচ করতে পারে এবং সাথে গার্ড ক্লজ (`when`) সাপোর্ট করে।
...
### প্রধান সুবিধাসমূহ:
1. **Exhaustiveness**: `PaymentMethod` যেহেতু একটি `sealed interface`, compiler নিশ্চিত করে সব সাবটাইপ কাভার করা হয়েছে কিনা। তাই কোনো `default` ব্রাঞ্চের প্রয়োজন পড়ে না।
...
Java 21-এর অন্যতম রোমাঞ্চকর ফিচার হলো **Record Patterns**। এটি কোনো রেকর্ডের ভেতর থেকে তার কম্পোনেন্টগুলোকে সরাসরি ভেঙে (Deconstruct) আলাদা variable-এ বের করে আনে।
...
> [!TIP]
> Record deconstruction কোডের বয়লারপ্লেট গেটার কল সম্পূর্ণ দূর করে। জটিল নেস্টেড object গ্রাফ থেকে ডেটা এক্সট্রাক্ট করা এখন এক লাইনেই অত্যন্ত পরিষ্কারভাবে সম্ভব।
