...
জাভা একটি object-ওরিয়েন্টেড ভাষা যেখানে সবকিছু object ও class-এর ধারণাকে কেন্দ্র করে ডিজাইন করা হয়েছে। বাস্তব জীবনের উপাদানসমূহকে (যেমন ইউজার, ব্যাংক অ্যাকাউন্ট, ইনভেন্টরি আইটেম) তাদের State / Fields এবং Behavior / Methods সহ কোডে মডেলিং করাই Object-Oriented Programming (OOP)-এর মূল উদ্দেশ্য।
...
OOP প্যারাডাইমে ডেটা থাকে সিস্টেমের কেন্দ্রে এবং method-গুলো সেই ডেটাকে সুরক্ষিত বাউন্ডারির ভেতরে নিয়ন্ত্রণ করে।
...
└────────────────────┘                 └───────────────────────────────┘
```
1. `BankAccount acc`: Stack memory-তে একটি reference variable তৈরি হয় যা heap-এর memory অ্যাড্রেস ধরে রাখে।
2. `new BankAccount(...)`: Heap memory-তে object-টির জন্য memory বরাদ্দ হয় এবং ফিল্ডগুলো ইনিশিয়ালাইজ হয়।
...
> - primitive type পাস করলে: মানের সরাসরি Copy of the value যায়।
> - object রেফারেন্স পাস করলে: রেফারেন্সের অ্যাড্রেসের একটি Copy of the reference যায়।
...
> [!TIP]
> `numbers.length` হলো একটি ফিল্ড/প্রোপার্টি (ব্র্যাকেট ছাড়া), অন্যদিকে `string.length()` হলো একটি method। এই পার্থক্যটি পরীক্ষায় বহুল পরিচিত একটি প্রশ্ন।
...
1. একটি method-এ **শুধুমাত্র একটি** vararg parameter থাকতে পারে।
2. Vararg প্যারামিটারটিকে অবশ্যই method parameter লিস্টের **সর্বশেষ উপাদান** হতে হবে (`(int... nums, String name)` compile error দেবে; `(String name, int... nums)` বৈধ)।
...
> [!WARNING]
> যদি রিকার্শনে **Base Case** সঠিকভাবে সংজ্ঞায়িত না থাকে বা রিকার্শন খুব গভীর হয়, তবে stack memory পূর্ণ হয়ে JVM ক্র্যাশ করবে এবং **`java.lang.StackOverflowError`** ঘটবে।
...
### প্যাকেজের সুবিধা:
1. **নেমিং কনফ্লিক্ট প্রতিরোধ**: দুটি ভিন্ন প্যাকেজে একই নামের class (যেমন `java.util.Date` এবং `java.sql.Date`) নিরাপদে সহাবস্থান করতে পারে।
2. **অ্যাক্সেস কন্ট্রোল**: Package-private লেভেলে class-এর অভ্যন্তরীণ উপাদানগুলোকে প্যাকেজের বাইরে সম্পূর্ণ লুকানো যায়।
...
| Modifier | একই class | একই প্যাকেজ | সাব-class (অন্য প্যাকেজে) | বাইরের যেকোনো প্যাকেজ |
| :--- | :---: | :---: | :---: | :---: |
| `private` | ✅ | ❌ | ❌ | ❌ |
...
## ১২. Static vs Instance Members

- **instance member**: প্রতিটি object-এর জন্য memory-তে আলাদা কপি তৈরি হয়।
- **static member**: সম্পূর্ণ class-এর জন্য memory-তে একটিমাত্র শেয়ার্ড কপি থাকে যা class-এর নাম দিয়ে সরাসরি এক্সেস করা যায় (`ClassName.method()`)।
...
encapsulation হলো object-এর অভ্যন্তরীণ ফিল্ডসমূহকে `private` রেখে সরাসরি বাহ্যিক মডিফিকেশন থেকে গোপন রাখা এবং নিয়ন্ত্রিত পাবলিক method-এর (Getters & Setters) মাধ্যমে বিজনেস ভ্যালিডেশন নিশ্চিত করা।
...
### কখন একটি object GC-র জন্য যোগ্য (Eligible for GC) হয়:
একটি object তখনই GC-র জন্য এলিজিবল হয় যখন অ্যাপ্লিকেশনের কোনো লাইভ thread থেকে তার কাছে পৌঁছানো যায় না (**Unreachable**)।
...
যদি দুটি object পরস্পরকে রেফার করে (`A.ref = B; B.ref = A;`), কিন্তু বাইরের কোনো সক্রিয় রেফারেন্স তাদের ধরে না রাখে, তবে পুরো গ্রুপটি আনরিচেবল গণ্য হয়ে GC দ্বারা ডিলিট হয়ে যায়।
...
- **Stack vs Heap**: রেফারেন্স থাকে Stack-এ, মূল object থাকে Heap-এ।
- **Parameter Passing**: জাভা সর্বদা ১০০% **Pass-by-Value**। object রেফারেন্স পাস করলেও memory-র রেফারেন্স pointer-টি কপি হয়।
- **Arrays & Varargs**: array fixed size object; varargs (`type...`) ব্যাকগ্রাউন্ডে array হিসেবে compile হয়।
