...
Java Collections Framework (JCF) হলো ডেটা স্টোর, প্রসেস এবং ম্যানিপুলেট করার জন্য একটি সমন্বিত স্থাপত্য। সাধারণ অ্যারের ফিক্সড সাইজের সীমাবদ্ধতা দূর করে ডায়নামিক মেমরি ম্যানেজমেন্ট ও বহুল ব্যবহৃত ডেটা স্ট্রাকচার (যেমন ডায়নামিক অ্যারে, লিঙ্কড লিস্ট, হ্যাশ টেবিল, ট্রি) এক্সেস করতে এটি ব্যবহৃত হয়।
...
> [!important]
> মনে রাখবে `Map` কোনো `Collection` বা `Iterable` ইন্টারফেসকে ইনহেরিট করে না; এটি কী-ভ্যালু পেয়ারের সম্পূর্ণ স্বাধীন একটি ভিন্ন হায়ারার্কি।
...
| :--- | :--- | :--- |
| **অভ্যন্তরীণ ডেটা স্ট্রাকচার** | রিসাইজেবল ডায়নামিক অ্যারে | ডাবলি লিঙ্কড লিস্ট (Doubly Linked List) |
| **ইনডেক্স এক্সেস (`get(i)`)** | **O(1) তাত্ক্ষণিক** | O(n) ধীর (নোড ট্রাভার্স করতে হয়) |
| **শুরুতে ডেটা ইনসার্ট/ডিলিট** | O(n) (এলিমেন্ট শিফট করতে হয়) | **O(1) তাত্ক্ষণিক** |
| **মেমরি লোকালিটি (Cache)** | **চমৎকার (কনটিগুয়াস মেমরি)** | খারাপ (র্যামের ছড়ানো নোড পয়েন্টার) |
...
1. **বাকেট অ্যারে (Bucket Array)**: ডিফল্টভাবে ১৬টি বাকেট নিয়ে ইনিশিয়ালাইজ হয়।
2. **হ্যাশিং ও ইনডেক্সিং**: `put(key, value)` কল করলে কী-এর `hashCode()` মেথড কল হয় এবং বিটওয়াইজ অপারেশনের মাধ্যমে বাকেট ইনডেক্স নির্ধারিত হয়।
3. **কলিশন রেজোলিউশন (Collision Resolution)**: যদি দুটি আলাদা কী-এর হ্যাশ একই বাকেট ইনডেক্সে পড়ে, তবে সেখানে লিঙ্কড লিস্ট নোড হিসেবে যুক্ত হয়।
...
## ৫. আধুনিক ইমিউটেবল কালেকশন ফ্যাক্টরি (Java 9+)

Java 9 থেকে অত্যন্ত পরিচ্ছন্ন এক লাইনে আনমডিফায়েবল ইমিউটেবল কালেকশন তৈরি করা যায়:
...
- **`Comparable`**: অবজেক্টের নিজস্ব ক্লাসের ভেতর `compareTo()` মেথড দিয়ে ন্যাচারাল সর্টিং ডিফাইন করা।
...
- অবজেক্টকে ম্যাপে নিরাপদে ব্যবহার করতে `equals()` ও `hashCode()` নিখুঁত হতে হবে।
- আধুনিক Java-তে `List.of()`, `Set.of()` দিয়ে দ্রুত ইমিউটেবল কালেকশন বানানো যায়।
