...
```rust
// Conceptual test harness execution loop:
...
cargo test test_add     # Filter tests by substring match
cargo test -- --nocapture  # Print stdout during test execution
cargo test -- --test-threads=1  # Run tests sequentially (single thread)
```
...
```rust
// Expansion of assert_eq! macro:
{
    match (&add(2, 3), &5) {
        (l, r) if *l == *r => {}   // Equal: assertion passes without panic
        _ => panic!(
            "assertion `left == right` failed\n  left: {:?}\n right: {:?}",
            l, r
        ),   // Includes file!() and line!() failure diagnostics
    }
}
```
...
pub fn index_element(v: &[i32], i: usize) -> i32 {
    v[i]  // Panics on out-of-bounds indexing
}
...
cargo doc --open        # Generate documentation and view in browser
```
...
│   └── #[cfg(test)]
│       mod tests {
│           // Private helper testable within child test module
│       }
tests/
├── integration.rs
│   // Tests external public crate interface
└── common/
    └── mod.rs
```
