...
```rust
// Conceptual state machine generated by compiler for async fn
enum FetchData {
    Start,                  // Not polled yet
    Waiting(InnerFuture),   // Suspended at first await point
}
...
impl Future for FetchData {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<String> {
        match self {
            // Start: initiate task; register waker and return Pending if not ready
            // Waiting: poll underlying future; transition to next state upon Ready
        }
    }
}
```
...
```rust
// Desugared #[tokio::main] runtime entry point:
fn main() {
...
#[tokio::main]
async fn main() {
    // Detached background task running concurrently:
    ...
    // Await task completion and collect result:
...
// Pinning guarantees pointer stability across yield points:
// Pinned future memory cannot move once polled
// Prevents invalidation of internal self-references
```
