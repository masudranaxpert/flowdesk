# Rust কী ও কেন শিখবে

Rust হলো একটি আধুনিক systems programming language যা C এবং C++ এর মতো সমান গতিসম্পন্ন ও দক্ষ, কিন্তু সাথে দেয় একটি যুগান্তকারী নিশ্চয়তা — **Garbage Collector (GC) ছাড়াই compile-time memory safety**। Mozilla ২০১০ সালে এটি প্রথম তৈরি করে এবং Stack Overflow-এর বৈশ্বিক ডেভেলপার সার্ভেতে Rust টানা ৯ বছর ধরে **বিশ্বের সবচেয়ে প্রশংসিত ভাষা** (Most Loved Programming Language)।

---

## ১. Rust কেন শিখবে?

ঐতিহ্যগত সিস্টেম প্রোগ্রামিং ভাষাগুলোতে (যেমন C বা C++) memory ম্যানেজমেন্ট সম্পূর্ণ প্রোগ্রামারকে নিজে হাতে (`malloc`/`free`) করতে হয়। এর ফলে সামান্যতম ভুলে তৈরি হয়:
- **Memory Leak**: memory ব্যবহার শেষে রিলিজ না করা।
- **Dangling Pointer / Use-After-Free**: মুছে ফেলা memory অ্যাক্সেস করে ক্র্যাশ বা সিকিউরিটি হোল।
- **Data Race**: মাল্টি-থ্রেডিংয়ে একই memory-তে একসাথে একাধিক thread-এর পরিবর্তন।

Python বা Go-এর মতো আধুনিক ভাষাগুলোতে একটি ব্যাকগ্রাউন্ড **Garbage Collector (GC)** থাকে যা নিয়মিত memory স্ক্যান করে খালি করে। কিন্তু এর ফলে অতিরিক্ত CPU overhead এবং অপ্রত্যাশিত pause তৈরি হয়।

Rust এই দ্বন্দ্ব সমাধান করেছে **Ownership ও Borrowing** সিস্টেমের মাধ্যমে। প্রোগ্রাম চলার সময় কোনো GC লাগে না; compiler নিজে বিল্ড করার সময়ই প্রতিটি variable-এর memory ব্যবহারের মেয়াদ নিশ্চিত করে।

```rust
fn main() {
    // Print a welcoming message to stdout
    println!("Hello, World!");
}
```

### প্রথম কোডের লাইন-বাই-লাইন বিশ্লেষণ:

1. **`fn main()`**: Rust প্রোগ্রামের এক্সিকিউশন শুরু হয় `main` function দিয়ে। `fn` কিওয়ার্ড দিয়ে function সংজ্ঞায়িত করা হয়।
2. **`{ ... }`**: function-এর সমস্ত কোড কার্লি ব্রেসের ভেতরে ব্লক হিসেবে আবদ্ধ থাকে।
3. **`println!("Hello, World!");`**:
   - `println!` সাধারণ function নয় — এটি একটি **macro** (নামের শেষে `!` দেখে শনাক্ত করা যায়)।
   - compile করার সময় Rust compiler (`rustc`) এই লাইনটিকে সম্প্রসারিত (expand) করে দক্ষ মেশিন কোডে রূপান্তর করে।
   - লাইনের শেষে সেমিকোলন (`;`) আবশ্যক, যা একটি স্টেটমেন্টের সমাপ্তি নির্দেশ করে।

---

## ২. variable ও ফরম্যাটিং সহ প্রিন্ট করা

Rust-এ টেক্সট ফরম্যাটিং অত্যন্ত সহজ ও শক্তিশালী:

```rust
fn main() {
    let language = "Rust";
    let version = 2024;
    
    // Positional interpolation using curly braces
    println!("স্বাগতম! আমি {} সংস্করণ শিখছি।", language);
    
    // Direct variable interpolation inside placeholder
    println!("ভাষা: {language}, এডিশন: {version}");
}
```

### কোডের লাইন-বাই-লাইন বিশ্লেষণ:
- **`let language = "Rust";`**: `let` দিয়ে variable ঘোষণা করা হয়। Rust স্বয়ংক্রিয়ভাবে এর টাইপ অনুমান (type inference) করে নেয়।
- **`{}` Placeholder**: `println!` ম্যাক্রোতে `{}` চিহ্নটি একটি ফরম্যাট স্পেসিফায়ার। কমার পরে থাকা variable-এর মান ক্রমানুসারে এই বন্ধনীগুলোর জায়গায় প্রতিস্থাপিত হয়।
- **`{language}`**: আধুনিক Rust-এ variable-এর নাম সরাসরি বন্ধনীর মধ্যে লিখলেও তা সুন্দরভাবে প্রিন্ট হয়।

---

## ৩. Python vs C++ vs Rust — তুলনা

| বিষয় | Python | C++ | Rust |
| :--- | :--- | :--- | :--- |
| **গতি (Speed)** | ধীর (Interpreted) | সর্বোচ্চ গতি | সর্বোচ্চ গতি (C++ এর সমকক্ষ) |
| **memory নিরাপত্তা** | নিরাপদ (GC আছে) | ঝুঁকিপূর্ণ (Manual free) | **নিরাপদ (GC ছাড়াই!)** |
| **concurrency** | GIL বাধা (Slow) | জটিল (Data race ঝুঁকি) | **Fearless Concurrency** |
| **টাইপ সিস্টেম** | Dynamic | Static | Static + Type Inference |
| **টুলিং** | pip + venv (আলাদা) | CMake/Make (জটিল) | **Cargo (অল-ইন-ওয়ান)** |

---

## ৪. কোথায় Rust সবচেয়ে বেশি ব্যবহৃত হচ্ছে?

- **Systems & Kernel**: Linux Kernel, Windows Components, Redox OS।
- **Cloud & High Performance Servers**: Discord, Cloudflare, Amazon AWS, Dropbox।
- **Fast CLI Tools**: `ripgrep` (grep এর চেয়ে ১০ গুণ দ্রুত), `fd` (find এর বিকল্প), `bat`।
- **WebAssembly (Wasm)**: ব্রাউজারে ভারী অ্যাপ্লিকেশন চালাতে (Figma, Photoshop Web)।
- **Modern Web Backends**: Actix-web, Axum (অত্যন্ত উচ্চ throughput ও নিম্ন latency)।

---

## ৫. Rust ইনস্টলেশন

Rust ইনস্টল ও ম্যানেজ করার অফিশিয়াল টুল হলো **`rustup`**।

### Linux ও macOS-এ:
টার্মিনালে নিচের কমান্ডটি রান করো:
```bash
curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh
```

### Windows-এ:
1. [rustup.rs](https://rustup.rs) থেকে `rustup-init.exe` ডাউনলোড করে চালান।
2. Visual Studio C++ Build Tools আবশ্যক।

### ইনস্টলেশন যাচাই:
```bash
rustc --version
cargo --version
```
যদি `rustc 1.8x.x` এবং `cargo 1.8x.x` দেখায়, তবে তোমার এনভায়রনমেন্ট পুরোপুরি প্রস্তুত।

---

## ৬. Cargo — Rust-এর শক্তিশালী টুলচেইন

Rust ইকোসিস্টেমে `cargo` একসাথে তিনটি মূল দায়িত্ব পালন করে:
1. **Package Manager**: লাইব্রেরি (crates) ডিপেন্ডেন্সি ডাউনলোড ও ম্যানেজ করে।
2. **Build System**: কোড অপটিমাইজ ও compile করে বাইনারি বানায়।
3. **Test Runner**: কোডের সমস্ত ইউনিট টেস্ট ও বেঞ্চমার্ক চালায়।

### নতুন প্রজেক্ট তৈরি ও স্ট্রাকচার:
```bash
cargo new hello_cargo
cd hello_cargo
```

তৈরি হওয়া ফাইল স্ট্রাকচার:
```text
hello_cargo/
├── Cargo.toml      # Project metadata and package dependencies
├── src/
│   └── main.rs     # Application entry point
└── target/         # Compiled build artifacts and binaries (gitignored)
```

### দৈনন্দিন প্রয়োজনীয় Cargo কমান্ডসমূহ:

| কমান্ড | কাজ ও উপযোগিতা |
| :--- | :--- |
| **`cargo check`** | কোডের syntax ও টাইপ ভ্যালিডেট করে। মেশিন কোড তৈরি করে না বলে **অত্যন্ত দ্রুত** কাজ করে। |
| **`cargo build`** | ডেভেলপমেন্ট বাইনারি তৈরি করে (`target/debug/hello_cargo`)। |
| **`cargo run`** | এক কমান্ডেই compile করে প্রোগ্রাম এক্সিকিউট করে। |
| **`cargo test`** | প্রজেক্টের সমস্ত টেস্ট রান করে। |
| **`cargo build --release`** | প্রোডাকশনের জন্য সর্বোচ্চ অপটিমাইজড বাইনারি তৈরি করে (`target/release/`)। |

---

## ৭. ইন্টারঅ্যাক্টিভ ইউজার ইনপুট প্রোগ্রাম

এবার টার্মিনাল থেকে ইউজারের নাম ইনপুট নিয়ে তাকে স্বাগতম জানানোর একটি সম্পূর্ণ প্রোগ্রাম দেখি:

```rust
use std::io;

fn main() {
    println!("তোমার নাম কী?");

    // Create a mutable, empty String buffer on the heap
    let mut name = String::new();

    // Read a line from standard input into the buffer
    io::stdin()
        .read_line(&mut name)
        .expect("Failed to read line");

    // Print greeting after stripping trailing newline characters
    println!("হ্যালো, {}! Rust প্রোগ্রামিংয়ে স্বাগতম।", name.trim());
}
```

### এই প্রোগ্রামের লাইন-বাই-লাইন পুঙ্খানুপুঙ্খ ব্যাখ্যা:

1. **`use std::io;`**:
   - Rust-এর স্ট্যান্ডার্ড লাইব্রেরির (`std`) ইনপুট/আউটপুট মডিউল `io` বর্তমান ফাইলে ইম্পোর্ট করা হয়েছে।
   - এর ফলে আমরা সরাসরি `io::stdin()` কল করতে পারি।
2. **`let mut name = String::new();`**:
   - `let` দ্বারা variable ঘোষণা করা হয়, কিন্তু Rust-এ variable ডিফল্টভাবে **immutable** (অপরিবর্তনযোগ্য)।
   - যেহেতু ইউজারের ইনপুট আসার পর variable-এর ভেতরে ডেটা রাখা হবে, তাই এটিকে পরিবর্তনশীল করতে **`mut`** কিওয়ার্ড বাধ্যতামূলক।
   - `String::new()` একটি খালি string বাফার তৈরি করে। এই মুহূর্তে heap memoryতে কোনো বাড়তি allocation হয় না (লেন্থ = 0, ক্যাপাসিটি = 0)।
3. **`io::stdin().read_line(&mut name)`**:
   - `io::stdin()` টার্মিনালের স্ট্যান্ডার্ড ইনপুট হ্যান্ডেল প্রদান করে।
   - `.read_line(...)` ইউজারের এন্টার চাপার আগ পর্যন্ত সমস্ত ক্যারেক্টার পড়ে বাফারে পুশ করে।
   - **`&mut name`**: memory-র মালিকানা হস্তগত না করে বাফারটিকে মডিফাই করার জন্য একটি **mutable reference** প্রদান করা হয়েছে।
   - `read_line` একটি `Result<usize, io::Error>` রিটার্ন করে, যা নির্দেশ করে সফলভাবে কত বাইট পড়া গেছে অথবা কোনো হার্ডওয়্যার/ওএস এরর হয়েছে কিনা।
4. **`.expect("Failed to read line");`**:
   - Rust-এ কোনো এরর অবহেলা করা যায় না।
   - `.expect()` method-টি `Result`-কে আনপ্যাক করে: সফল হলে বাইট সংখ্যা বের করে দেয়, আর এরর হলে প্রোগ্রাম বন্ধ (panic) করে টার্মিনালে মেসেজটি প্রদর্শন করে।
5. **`name.trim()`**:
   - ইউজার টাইপ করে Enter চাপলে ইনপুটের শেষে একটি নিউলাইন ক্যারেক্টার (`\n`) বা ক্যারেজ রিটার্ন যুক্ত থাকে।
   - `.trim()` method-টি কোনো নতুন memory কপি না করে শুধু শুরু ও শেষের অপ্রয়োজনীয় স্পেস এবং নিউলাইন বাদ দিয়ে একটি `&str` slice ভিউ রিটার্ন করে।

---

## ৮. Rust compiler কেন ডেভেলপারদের সবচেয়ে প্রিয়?

Rust-এর compiler (`rustc`) কেবল এরর ধরে না, বরং একজন ধৈর্যশীল শিক্ষকের মতো সমস্যাটির সঠিক সমাধানও বাতলে দেয়। উদাহরণস্বরূপ, যদি তুমি `mut` লিখতে ভুলে যাও:

```rust
fn main() {
    let x = 5;
    x = 6;
}
```

compiler সরাসরি নির্দেশ করবে:
```text
error[E0384]: cannot assign twice to immutable variable `x`
 --> src/main.rs:3:5
  |
2 |     let x = 5;
  |         - first assignment to `x`
  |         help: consider making this binding mutable: `mut x`
3 |     x = 6;
  |     ^^^^^ cannot assign twice to immutable variable
```

---

## সারাংশ

1. Rust হলো শূন্য ওভারহেড সহ memory নিরাপদ ভাষা — এতে কোনো Garbage Collector নেই।
2. `cargo` দিয়ে খুব সহজেই প্রজেক্ট তৈরি (`cargo new`), যাচাই (`cargo check`), এবং রান (`cargo run`) করা যায়।
3. ডিফল্টভাবে প্রতিটি variable immutable; পরিবর্তন করতে হলে `let mut` লিখতে হয়।
4. স্ট্যান্ডার্ড লাইব্রেরির যেকোনো function `use` কিওয়ার্ড দিয়ে ফাইলে এনে সহজে ব্যবহার করা যায়।